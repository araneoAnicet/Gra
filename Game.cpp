    ...
    while (!(hero->health->is_empty()) && !(enemy->health->is_empty())) {
        ...
        if (hero->weapon->cooldown->is_empty()) {
            if (hero->weapon->reach_distance->get() >= distance) {
                damage_enemy_receives = (1 - 0.01f * enemy->damage_resistance->get()) * hero->weapon->damage->get() + hero->experience->get() * 0.01f;
        ...
        if (enemy->weapon->cooldown->is_empty()) {
            if (enemy->weapon->reach_distance->get() >= distance) {
                damage_hero_receives = (1 - 0.01f * hero->damage_resistance->get()) * enemy->weapon->damage->get() + enemy->experience->get() * 0.01f;
